...
import org.apache.zookeeper.common.network.TransferableChannel;
...
public final class Utils {
    ...
    /**
     * Creates a map entry (for use with {@link Utils#mkMap(Entry[])})
     *
     * @param k   The key
     * @param v   The value
     ...
     * @return An entry
     */
    public static <K, V> Entry<K, V> mkEntry(final K k, final V v) {
     ...
     */
    @SafeVarargs
    public static <K, V> Map<K, V> mkMap(final Entry<K, V>... entries) {
    ...
    */
     ...
     */
    public static <K, V, M extends Map<K, V>> Collector<Entry<K, V>, M, M> entriesToMap(final Supplier<M> mapSupplier) {
        return new Collector<Entry<K, V>, M, M>() {
            @Override
            ...
            @Override
            public BiConsumer<M, Entry<K, V>> accumulator() {
